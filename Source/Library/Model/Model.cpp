...
)
{
	...
	hr = initMaterials(pDevice, pImmediateContext, pScene,
		filePath);
	if (FAILED(hr))
		return hr;
...
void Model::initAllMeshes(_In_ const aiScene*
	pScene)
{
	for (UINT i = 0u; i < m_aMeshes.size(); ++i)
	{
	...
	const std::filesystem::path& filePath)
{
	...
	for (UINT i = 0u; i < pScene->mNumMaterials; ++i)
	{
		...
		loadTextures(pDevice, pImmediateContext, parentDirectory,
			pMaterial, i);
...
{
	HRESULT hr = loadDiffuseTexture(pDevice, pImmediateContext,
		parentDirectory, pMaterial, uIndex);
	if (FAILED(hr))
		return hr;
	hr = loadSpecularTexture(pDevice, pImmediateContext, parentDirectory,
		pMaterial, uIndex);
	if (FAILED(hr))
		return hr;
	return hr;
...
HRESULT Model::loadDiffuseTexture(_In_ ID3D11Device* pDevice, _In_
	ID3D11DeviceContext* pImmediateContext, _In_ const std::filesystem::path&
	parentDirectory, _In_ const aiMaterial* pMaterial, _In_ UINT uIndex)
{
	HRESULT hr = S_OK;
	...
	parentDirectory, _In_ const aiMaterial* pMaterial, _In_ UINT uIndex)
{
	...
	if (pMaterial->GetTextureCount(aiTextureType_SHININESS) > 0)
	{
			...
			nullptr, nullptr, nullptr, nullptr) == AI_SUCCESS)
		{
			...
			hr = m_aMaterials[uIndex].pSpecular -> Initialize(pDevice,
				pImmediateContext);
			if (FAILED(hr))
			{
...
void Model::loadColors(_In_ const aiMaterial* pMaterial, _In_ UINT
	uIndex)
{
